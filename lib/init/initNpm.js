...
function createTemplate(dir, projectType, lang, template, graphqlIncludeConfirm) {
    if (lang === constants_1.language.javascript) {
        if (projectType === constants_1.typeProject.basicExpress && template === "none") {
            ...
            if (graphqlIncludeConfirm) {
                child_process_1.execSync("npm install express-graphql graphql", {
                    cwd: dir,
                });
            }
        }
        ...
        if (projectType === constants_1.typeProject.apiExpress) {
            ...
            if (graphqlIncludeConfirm) {
                child_process_1.execSync("npm install express-graphql graphql @types/graphql", {
                    cwd: dir,
                });
            }
        }
            ...
            template === "none") {
            generateTemplateTs_1.generateTypescriptTemplateBasic(dir);
        }
        else if (projectType === constants_1.typeProject.basicExpress &&
            template === constants_1.templateContentNodejs.ejs) {
            child_process_1.execSync("npm i ejs path", {
...
function startCmdProject(dir, event) {
    const { stderr } = child_process_1.exec(`npm run ${event}`, {
        cwd: dir,
    });
}
